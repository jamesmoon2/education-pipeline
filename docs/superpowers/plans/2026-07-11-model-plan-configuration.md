...
| 4 | 2026-07-12 | 093aa3a | 465 | 109 | 40 | Gate ran at code commit 093aa3a; this docs-only plan-record commit sits on top. +8 pytest (4.1), +4 vitest & +2 e2e (4.2). Both tasks landed as specified; RunPlanPanel/PlanStageRow reused unmodified (no second plan editor). Whole-wave review (Opus): READY TO MERGE, zero Critical/Important. Import vs. structured-create parity confirmed: both converge on `save_topic_toml` → `_artifact_id` validation and `ConflictError("already_exists")`→409; structured path does NOT bypass id validation. `emit_topic_toml` round-trip safe (json.dumps escaping ⊂ TOML basic-string escapes). NOTE (pytest count): measured 465 passed/0 failed directly (457 + 8); a 4.1 implementer report said "471 collected" — that was a misreport, disregard it. Ambiguity resolutions in 4.2: TOML-mode calls `importTopic` client fn directly (needs returned id to advance the wizard); no-profiles auto-skip; run-init (`postAdvance`) awaited before RunPlanPanel mounts (a race here was caught+fixed by e2e). **Wave 5 triage (all deferred, none block):** (a) `create_topic` body-validation helpers duplicate topics.py `_optional_string`/`_string_tuple`; (b) no non-empty-`metadata` round-trip test AND latent bug — `emit_topic_toml`'s `[metadata]` branch emits nested-dict values via bare json.dumps → invalid TOML inline-table (unreachable: create_topic never sets metadata; cover both when tested); (c) `NewRunPage.enterPlanStep` advances to the plan step even when `postAdvance` throws (useAction.run swallows the error) — visible/recoverable, not silent, but should halt; (d) e2e verifies plan rows loosely (heading + 1 combobox); (e) `createTopic` has no fetch-layer client.test.ts case (matches repo convention). Carry-forward Wave-3 global-edit-invalidates-override hazard NOT newly exposed (wizard sets no overrides). |
| 5 | 2026-07-12 | 0d5f9f8 | 478 | 114 | 41 | MILESTONE CLOSED. Gate ran at code commit 0d5f9f8; this docs-only plan-record commit sits on top. +13 pytest (5.1:0 e2e-only, 5.2:2, MUST-FIX:7, fix batch:4), +5 vitest, +1 e2e vs Wave 4. Wave 5 delivered: (1) `web/e2e/model-plan.spec.ts` — mixed-provider acceptance flow (recommended defaults + draft→codex override + qa manual, exact provenance assertions, PATH-stubbed claude/codex CLIs, no TOML in the UI path); (2) TOML hand-edit round-trip regression tests over a real `WorkspaceConfigSource`; (3) the Wave-3 MUST-FIX resolved: `apply_overrides_lenient` (config.py) degrades the run plan per stage — GET `/v1/runs/{t}/plan` never 400s on stored-override content, the broken stage carries `override_error` (rendered `role="alert"` in PlanStageRow) with `command: null`, enqueue/execution fail explicitly for that stage only, and PUT rejects only stages the request touches, so per-row "Use recommended" is the in-UI recovery; (4) final-review FIX-NOW batch: duplicate `manual` option gated, malformed overrides file (array / non-mapping stages) → 400 not 500, next-stage line handles finalize/export + "manual" de-doubled, NewRunPage no longer advances past a failed run-init (`useAction.run` now returns `Promise<boolean>`); (5) README cockpit docs + PRD §10 P0 closed. Whole-wave review (Fable): READY TO RECORD, zero Critical/Important. Frozen-surface guard passed (no guides/guide_runtime changes since 7b91f41; fixture SHA untouched). Deferred-minors triage: FIX-NOW items all landed; ACCEPT + NEXT-MILESTONE items recorded in the post-milestone audit doc. |
...
- [x] **Step 1: Failing tests** — emit/parse round-trip for a Topic with goals + special characters; POST without `toml` creates the topic file and a follow-up GET `/v1/topics/{id}` returns it; duplicate id without overwrite → 409; missing `title` → 400.
- [x] **Step 2: Implement, verify, commit** — `git commit -m "feat(daemon): create topics from structured fields"` (commit 6c9a7aa)
...
- [x] **Step 1: Failing vitest tests** — form submit calls `createTopic` with parsed fields; TOML mode calls the import client; empty TopicListPage shows the `/new` link.
- [x] **Step 2: Implement.**
- [x] **Step 3: E2E** — `web/e2e/new-run.spec.ts` (copy the daemon-fixture bootstrapping from `smoke.spec.ts`): from an empty workspace, click through New run → describe a topic → skip profile → see the plan review with per-stage rows → land on the run board. Include an `@axe-core` accessibility pass on `/new` matching the pattern in the existing specs.
- [x] **Step 4:** Full web gate (`npm test -- --run`, `npm run build`, `npm run e2e`); commit `feat(web): new-run wizard replacing the paste-TOML empty state` (commit 093aa3a).

### Wave 4 exit gate

- [x] Full gate; Wave Log; commit plan file. **Done: pytest 465, vitest 109, build clean, e2e 40. Whole-wave review (Opus): READY TO MERGE, zero Critical/Important.**
...
- [x] **Step 1:** Write the spec covering, in one flow and **without ever editing TOML in the UI-driven path**:
...
- [x] **Step 2:** Make it pass; commit `test(e2e): mixed-provider run configured entirely in the cockpit`.
...
- [x] **Step 2:** Commit `test(daemon): TOML round-trip regression for plan edits`.

### Task 5.3: Docs, PRD status, closeout

- [x] Update `docs/product-requirements.md` §10: mark "P0 — Finish model-plan configuration" complete with a one-line pointer to this plan.
