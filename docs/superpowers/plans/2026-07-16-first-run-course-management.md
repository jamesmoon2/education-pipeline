...
| 2 — `ui` launcher | **complete** | `49be5e4` | 1040 (+1 skip) | n/a | n/a | n/a | `run_ui(workspace, no_browser=, deps=UiDeps(...))` frozen; exit codes: 2 `workspace_unselected`, 1 `workspace_invalid`/`web_assets_missing`. Auto-fix applies only when ALL findings are auto-fixable. Asset check happens before daemon start. Packaging smoke verified locally against the real wheel (index served, daemon stop clean). CI gains `packaging-smoke` job. |
| 3 — Course library | **complete** | `8193e87` | 1086 (+1 skip) | 239 | 53 | clean | Guard covers advance/ingest/edit/approve/validate/waivers/plan-overrides/audit-prepare/finalize/export/attach + provider enqueue; guard lives at the daemon adapter layer (direct CLI RunStore calls are not guarded — cockpit is the archive surface). `write_api.attach_profile` gained a `runs` param. Reveal statuses: 200 `{path}`, 422 `reveal_unsupported` + `detail.path`, 400 bad enum. Duplicate route returns 201. `EP_REVEAL_OPENER` is the opener test hook. `filterAndSortTopics` exported from TopicListPage for unit reuse. |
| 4 — Onboarding + wizard | **complete** | `bb37b66` | 1089 (+1 skip) | 249 | 53 | clean | Welcome dismissal key `ep.welcome.dismissed`; `resetWelcomeDismissal()` exported for Settings. Wizard steps are table-driven (`STEP_ORDER`) with create-at-confirm and idempotent retry (`createdId`/`attached` markers). `web/e2e/new-run.spec.ts` rewritten for the four-step flow + welcome assertions. Env note: after a session restart, Playwright 1.61 wanted chromium 1228; shimmed `/opt/pw-browsers/chromium_headless_shell-1228` → preinstalled chromium 1194 locally (CI installs its own browsers; no repo change). |
| 5 — Acceptance + closeout | **complete** | (this commit) | 1089 (+1 skip) | 249 | 56 | clean | Final gate run 2026-07-16 (e2e 56/56 in two consecutive clean full runs; one earlier run overlapping vitest+build showed a single load-induced failure and was rerun). New `web/e2e/library.spec.ts` (archive/filter/unarchive, duplicate-at-spec, reveal copyable-path fallback via `EP_REVEAL_OPENER=/bin/false`); first-run welcome→create→run-board flow lives in the rewritten `web/e2e/new-run.spec.ts`. README documents `ui`, the registry location, and `workspace check`. PRD §10 entry moved to Delivered; post-milestone audit ledger recorded at `docs/superpowers/specs/2026-07-16-first-run-course-management-post-milestone-audit.md`. Packaging smoke verified locally against the built wheel in Wave 2 and covered in CI by the `packaging-smoke` job. |
