...
| T21 | Frame/module draft prompts and assembly | `compile_guide_v1_frame_draft_prompt`, `compile_guide_v1_module_draft_prompt`; `assemble_guide(frame, modules, module_ids)` in `canonical.py` is byte-deterministic and refuses id collisions, renames, missing or extra stubs; prompt snapshot tests. | - [x] |
...
- **T21** landed: `guides/canonical.py` gains `validate_frame` (lenient shape check, never `parse_guide`) and `assemble_guide`, with `splice_module` now sharing `_module_fragment` and `_reparse_strictly` (canonical.py 126 → 303 lines); `prompts.py` gains `compile_guide_v1_frame_draft_prompt` and `compile_guide_v1_module_draft_prompt` (1582 → 1906), reusing the draft and module-repair helpers; `compile_guide_v1_draft_prompt` byte-identical. 48 new tests (31 canonical, 17 prompts); suite 1823 passed / 1 skipped. Process note: the first test-writer and implementer dispatches overlapped because the manager's wait loop exited early; the fabricated-looking interim reports were discarded and the thread was re-run with diff-stat evidence required. Accepted: the four new names are module-level only, matching `splice_module` and the other compilers, which are not re-exported either.
