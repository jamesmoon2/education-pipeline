...
| 08 | Wave 7 | CI, packaging, accessibility, docs, closeout | Complete | `docs/superpowers/prompts/interactive-guide-v1/chunk-08-supervisor.md` |
| 09 | Post-milestone | Independent audit and next-milestone proposal | Ready | `docs/superpowers/prompts/interactive-guide-v1/chunk-09-post-milestone-supervisor.md` |
...
### Chunk 08 completion record

- **Completed:** 2026-07-11. Chunk 08 and the milestone are `Complete`.
- **Blocker resolution:** Blocker 1 (skip link) was fixed under separate owner
  authorization in `e2cb6f4` with a new keyboard e2e test. Blocker 2 was
  closed by explicit owner sign-off: the owner manually exercised the cockpit,
  editing flow, export, and downloads, and **waived the remaining
  human-required accessibility items without execution** (screen-reader smoke,
  formal keyboard checklist, real print dialog, real-device reflow); see the
  owner sign-off section of
  `docs/testing/2026-07-11-interactive-guide-v1-acceptance.md`. Those items
  are recommended before a public release and are flagged for the Chunk 09
  audit.
- **Accepted commits:** `7b2afd1` (CI web + e2e jobs), `a766674` (guide-v1
  user documentation), `9f55142` (acceptance evidence and blockers),
  `e2cb6f4` (skip-link fix + e2e test), `9be67d8` (fix records),
  `20d9a74` (stage/editor action toolbars, owner-requested UX change),
  `434d11e` (smoke-spec daemon discovery race fix), plus this closeout.
- **Final verification (2026-07-11):** `git diff --check` clean;
  `python3 -m pytest` — 404 passed; `cd web && npm test` — 79 passed;
  `npm run build` — passed; `npm run e2e` — 38 passed (twice consecutively).
  Wheel, clean-install, `file:` export, and fixture-hash evidence as recorded
  in the status record above; normalized fixture SHA-256 unchanged:
  `99fde906c6bb1231c33c4d5d9f1adab011a1f4313c03c574eb7aa27cdbe70b07`.
- **Post-baseline intentional changes:** the skip-link runtime fix, the
  stage/editor action toolbar relocation, and the smoke-spec bootstrap fix
  were each owner-authorized during acceptance; unit suite grew 78→79 and
  browser acceptance suite 37→38. No other frozen surface changed.
- **Remaining risks:** waived manual accessibility items above; blueprint and
  course-brief schemas delivered only to the depth the fixture requires
  (recorded in the PRD as a partial); cockpit discoverability gaps observed
  by the owner (empty-board first impression, preview hidden behind the
  editor, finalized-run read-only state unexplained) — all handed to the
  Chunk 09 audit.
