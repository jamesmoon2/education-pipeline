...
- On start, generate a random token; write it plus the port to
  `.education-pipeline/daemon.json` (mode `0600`): `{pid, port, token,
  started_at, version}`. Clients read this file to locate and authenticate.
- Every request must present the token (header, e.g. `X-EP-Token`); reject
  otherwise. This stops other local processes and stray browser tabs / random
  localhost pages from driving runs.
...
- **Commands**: `daemon start` (spawn detached), `daemon status` (PID alive +
  ...
  in-flight job per flag, exit, remove the file).
...
- **Orphan reconciliation**: on startup the daemon marks any job left
  `running`/`queued` from a previous life whose process is gone as
  `interrupted`, with a note, and writes **no** partial response.
...
- **Cancellation**: SIGTERM the process group, then SIGKILL after a grace
...
- **Lifecycle/recovery**: start/stop, stale-file replacement, orphan
  ...
  response).
- **CLI**: `run` refusing when the next action is an approval; `manual` provider
  returning the do-it-yourself message.
- **Auth**: requests without the token are rejected; `Origin`/`Host` allowlist
  enforced.
