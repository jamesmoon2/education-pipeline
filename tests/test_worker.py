...
def test_job_read_retries_transient_windows_sharing_violation(tmp_path, monkeypatch):
    """A reader that opens job.json mid-atomic-replace gets PermissionError
    on Windows (delete-pending file). The store must retry briefly instead
    of surfacing a 500 through whichever HTTP handler happened to be listing
    jobs at that moment."""
    ...
    def flaky_read_text(self, *args, **kwargs):
        if self.name == "job.json" and failures["remaining"] > 0:
            failures["remaining"] -= 1
            raise PermissionError("sharing violation")
        return real_read_text(self, *args, **kwargs)

    monkeypatch.setattr(Path, "read_text", flaky_read_text)
    assert [j.id for j in store.all_jobs()] == [job.id]
    assert failures["remaining"] == 0
    assert store.load("t", job.id).id == job.id


def test_job_save_retries_transient_windows_sharing_violation(tmp_path, monkeypatch):
    """os.replace onto job.json fails with PermissionError while a concurrent
    reader holds the file open on Windows; the save must retry, not fail the
    job with a spurious 'unexpected error'."""
    ...
    def flaky_replace(src, dst):
        if str(dst).endswith("job.json") and failures["remaining"] > 0:
            failures["remaining"] -= 1
            raise PermissionError("sharing violation")
    ...
    store.save(job)
    assert failures["remaining"] == 0
    assert store.load("t", job.id).status == "running"
